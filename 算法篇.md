...
class NumArray {
  ...
  constructor(nums: number[]) {
    ...
    this.nums = nums
    this._build(1, 0, this.size - 1)
  }

  _build(root = 1, l = 0, r = this.size - 1): void {
    if (l === r) {
  ...
  _query(min: number, max: number, l = 0, r = this.size - 1, root = 1) {
    if (min <= l && r <= max) {
    ...
    let res = 0
    const mid = Math.floor((l + r) / 2)
    if (min <= mid) res += this._query(min, max, l, mid, root << 1)
    if (mid + 1 <= max) res += this._query(min, max, mid + 1, r, (root << 1) | 1)
    return res
  }

  // 单点修改
  _updateSingle(p: number, target: number, l = 0, r = this.size - 1, root = 1) {
    if (l === p && p === r) {
    ...
    const mid = Math.floor((l + r) / 2)
    if (p <= mid) this._updateSingle(p, target, l, mid, root << 1)
    if (mid + 1 <= p) this._updateSingle(p, target, mid + 1, r, (root << 1) | 1)
    this._pushUp(root)
  }

  update(index: number, val: number): void {
    this._updateSingle(index, val)
  }

  sumRange(left: number, right: number): number {
    return this._query(left, right)
  }
}
```
