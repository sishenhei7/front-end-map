...
class NumArray {
  ...
  constructor(nums: number[]) {
    const len = nums.length
    this.tree = Array(len << 2)
    // this.tree = new Uint32Array(len << 2)
    this.nums = nums
    this._build(1, 0, len - 1)
  }

  _build(root: number, l: number, r: number): void {
    if (l === r) {
    ...
    this._build(root << 1, l, mid)
    this._build((root << 1) | 1, mid + 1, r)
    this._pushUp(root)
  }
  ...
  _query(root: number, l: number, r: number, min: number, max: number) {
    if (min <= l && r <= max) {
    ...
    let res = 0
    const mid = Math.floor((l + r) / 2)
    if (min <= mid) res += this._query(root << 1, l, mid, min, max)
    if (mid + 1 <= max) res += this._query((root << 1) | 1, mid + 1, r, min, max)
    return res
  }

  // 单点修改
  _updateSingle(root: number, l: number, r: number, p: number, target: number) {
    if (l === p && p === r) {
    ...
    const mid = Math.floor((l + r) / 2)
    if (p <= mid) this._updateSingle(root << 1, l, mid, p, target)
    if (mid + 1 <= p) this._updateSingle((root << 1) | 1, mid + 1, r, p, target)
    this._pushUp(root)
  }

  update(index: number, val: number): void {
    this._updateSingle(1, 0, this.nums.length - 1, index, val)
  }

  sumRange(left: number, right: number): number {
    return this._query(1, 0, this.nums.length - 1, left, right)
  }
}
```
